    ...
    <div class="max-w-md mx-auto bg-white shadow-sm border border-gray-100 p-6 sm:p-8 rounded-2xl shadow-gray-50/50">
        ...
        <form method="POST" action="{{ route('register.submit') }}" enctype="multipart/form-data" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name') }}" class="w-full border border-gray-300 rounded-xl px-4 py-2.5 text-sm font-medium focus:ring-2 focus:ring-primary-500 focus:border-primary-500 shadow-sm @error('name') border-red-500 @enderror" placeholder="Masukkan nama lengkap beserta gelar" required autofocus>
            ...
            <div>
                ...
                <input type="email" name="email" id="email" value="{{ old('email') }}" class="w-full border border-gray-300 rounded-xl px-4 py-2.5 text-sm font-medium focus:ring-2 focus:ring-primary-500 focus:border-primary-500 shadow-sm @error('email') border-red-500 @enderror" placeholder="contoh@email.com" required>
            ...
            <div>
                ...
                <input type="text" name="nip" id="nip" value="{{ old('nip') }}" class="w-full border border-gray-300 rounded-xl px-4 py-2.5 text-sm font-medium focus:ring-2 focus:ring-primary-500 focus:border-primary-500 shadow-sm @error('nip') border-red-500 @enderror" placeholder="1995xxxxxxxxxxxxxx" required>
                @error('nip') <p class="mt-1.5 text-xs text-red-600 font-semibold">{{ $message }}</p> @enderror
            </div>
            ...
            <div>
                ...
                <input type="text" name="asal_instansi" id="asal_instansi" value="{{ old('asal_instansi') }}" class="w-full border border-gray-300 rounded-xl px-4 py-2.5 text-sm font-medium focus:ring-2 focus:ring-primary-500 focus:border-primary-500 shadow-sm @error('asal_instansi') border-red-500 @enderror" placeholder="Contoh: SDN 01 Padang" required>
                @error('asal_instansi') <p class="mt-1.5 text-xs text-red-600 font-semibold">{{ $message }}</p> @enderror
            </div>
                ...
                <div class="pt-2 border-t border-gray-100 mt-4">
                    @foreach($bimtek->syaratDokumens as $syarat)
                        <div class="mb-4">
                            ...
                            <input type="file" name="dokumen[{{ $syarat->id }}]" id="dokumen_{{ $syarat->id }}" accept=".pdf,.jpg,.jpeg,.png" class="w-full border border-gray-300 rounded-xl px-4 py-2 text-sm font-medium bg-gray-50 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-primary-50 file:text-primary-700 hover:file:bg-primary-100 shadow-sm @error('dokumen.'.$syarat->id) border-red-500 @enderror" {{ $syarat->is_wajib ? 'required' : '' }}>
                            
                            @if($syarat->deskripsi_syarat)
                                <p class="mt-1 text-[11px] text-gray-400 font-medium leading-relaxed">{{ $syarat->deskripsi_syarat }}</p>
                            @endif
                            @error('dokumen.'.$syarat->id) <p class="mt-1.5 text-xs text-red-600 font-semibold">{{ $message }}</p> @enderror
                        </div>
                    @endforeach
                    <p class="mt-3 text-[10px] text-gray-400 font-medium leading-relaxed italic">* Format berkas dokumen yang didukung: PDF, JPG, atau PNG dengan ukuran maksimal masing-masing 5MB.</p>
                </div>
            @endif
            ...
            <div class="pt-4 border-t border-gray-100 mt-2">
                ...
                <input type="password" name="password" id="password" class="w-full border border-gray-300 rounded-xl px-4 py-2.5 text-sm font-medium focus:ring-2 focus:ring-primary-500 focus:border-primary-500 shadow-sm @error('password') border-red-500 @enderror" placeholder="Minimal 8 karakter" required>
            ...
            <div>
                ...
                <input type="password" name="password_confirmation" id="password_confirmation" class="w-full border border-gray-300 rounded-xl px-4 py-2.5 text-sm font-medium focus:ring-2 focus:ring-primary-500 focus:border-primary-500 shadow-sm" placeholder="Ulangi kembali kata sandi Anda" required>
            </div>
