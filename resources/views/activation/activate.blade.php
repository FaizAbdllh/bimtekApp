...
<div class="container mx-auto p-4">
    <div class="max-w-md mx-auto bg-white shadow p-6 rounded">
        <h2 class="text-xl font-semibold mb-4">Aktivasi Akun Peserta</h2>

        <form action="{{ url('/activate/'.$token) }}" method="post">
            @csrf
            <div class="mb-3">
                <label class="block text-sm">NIP (opsional)</label>
                <input name="nip" value="{{ old('nip', $user->nip ?? '') }}" class="mt-1 block w-full border rounded p-2">
            ...
            <div class="flex items-center gap-3">
                <button class="bg-blue-600 text-white px-4 py-2 rounded">Aktivasi</button>
