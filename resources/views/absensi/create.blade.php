        ...
        <div>
            ...
            <nav class="flex mb-2" aria-label="Breadcrumb">
                <ol class="inline-flex items-center space-x-1 text-xs text-gray-400 font-medium">
                    ...
                    <li><span class="mx-1">/</span></li>
                    <li class="text-gray-800 font-bold">Buat Sesi</li>
                </ol>
            </nav>
                ...
                <form action="{{ route('bimtek.absensi.store', $bimtek->id) }}" method="POST">
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        <div class="bg-gray-50/80 rounded-xl border border-gray-100 p-4 shadow-inner shadow-gray-50">
                            <h3 class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-0.5">Nama Kelas Bimtek</h3>
                        ...
                        <div>
                            <label for="nama_sesi" class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-2">
                                Label / Nama Sesi Absensi <span class="text-red-500">*</span>
                            </label>
                            <input type="text" 
                                   name="nama_sesi" 
                                   id="nama_sesi"
                            ...
                            @enderror
                            <p class="mt-1.5 text-xs text-gray-400 font-medium">Gunakan nama yang jelas sebagai representasi lembar presensi sesi ini.</p>
                        </div>

                        {{-- Form Pilihan Opsi Status Sesi Absensi --}}
                        <div>
                            <label class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-2">
                                Status Gerbang Sesi <span class="text-red-500">*</span>
                            </label>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <label class="relative flex cursor-pointer rounded-xl border bg-white p-4 shadow-sm focus:outline-none transition-all duration-200 @error('status') border-red-500 @enderror">
                                    <input type="radio" name="status" value="terbuka" class="sr-only" {{ old('status', 'terbuka') === 'terbuka' ? 'checked' : '' }}>
                                    <span class="flex flex-1">
                                        <span class="flex flex-col">
                                            <span class="block text-sm font-bold text-gray-900">Langsung Buka (Terbuka)</span>
                                            <span class="mt-1 text-xs text-gray-400 font-medium leading-relaxed">Gerbang presensi aktif, peserta dapat langsung scan QR atau melakukan absensi.</span>
                                        </span>
                                    </span>
                                ...
                                {{-- Opsi Ditutup --}}
                                <label class="relative flex cursor-pointer rounded-xl border bg-white p-4 shadow-sm focus:outline-none transition-all duration-200 @error('status') border-red-500 @enderror">
                                    <input type="radio" name="status" value="ditutup" class="sr-only" {{ old('status') === 'ditutup' ? 'checked' : '' }}>
                                    <span class="flex flex-1">
                                        <span class="flex flex-col">
                                            <span class="block text-sm font-bold text-gray-900">Simpan Draf (Ditutup)</span>
                                            <span class="mt-1 text-xs text-gray-400 font-medium leading-relaxed">Sesi disimpan terlebih dahulu ke sistem, pembukaan gerbang diaktifkan nanti saat acara dimulai.</span>
                                        </span>
                                    </span>
                                    <svg class="h-5 w-5 text-primary-600 hidden flex-shrink-0 ml-2" viewBox="0 0 20 20" fill="currentColor">
                                        <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                    ...
                    <div class="px-6 py-4 bg-gray-50 border-t border-gray-100 flex items-center justify-end gap-3 rounded-b-2xl shadow-inner shadow-gray-50">
                        ...
                        <a href="{{ route('bimtek.absensi.index', $bimtek->id) }}" class="px-4 py-2 text-xs font-bold text-gray-500 hover:text-gray-800 uppercase tracking-wide transition">
                            Batal
                        </a>
