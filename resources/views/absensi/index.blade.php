                                                ...
                                                @if($isPeserta && $sesi->isOpen() && !in_array($sesi->id, $userAttendances))
                                                    ...
                                                    @if(in_array($modePelaksanaan, ['online', 'hybrid']))
                                                        <form action="{{ route('bimtek.absensi.hadir-online', [$bimtek, $sesi]) }}" method="POST" class="inline">
                                                            @csrf
                                                            <button type="submit" class="inline-flex items-center gap-1 px-3 py-1.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-xs font-medium transition">
                                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
