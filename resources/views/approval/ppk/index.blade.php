    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
                <div>
                    <h2 class="text-2xl font-bold text-gray-800">Pengajuan Bimtek (Pejabat PPK)</h2>
                    <p class="mt-1 text-sm text-gray-600">Review dan berikan persetujuan pagu anggaran belanja SBM kegiatan bimbingan teknis</p>
                </div>
            </div>
                    ...
                    <form method="GET" action="{{ route('approval.ppk.index') }}" class="flex flex-col sm:flex-row gap-4">
                        ...
                        <div class="flex gap-2">
                            ...
                            @if (request('search') || request('status'))
                                <a href="{{ route('approval.ppk.index') }}" class="inline-flex items-center px-4 py-2.5 bg-gray-100 border border-gray-200 rounded-xl font-semibold text-sm text-gray-700 hover:bg-gray-200 transition">
                                    Reset
                                </a>
                            @endif
                        </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse ($pengajuans as $pengajuan)
                                ...
                                <tr class="hover:bg-gray-50 transition-colors {{ $pengajuan->status === 'disetujui_kepala' ? 'bg-blue-50/60' : '' }}">
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        {{-- REFAKTORISASI: Menyematkan ID untuk kestabilan pembacaan parameter UUID --}}
                                        <a href="{{ route('approval.ppk.show', $pengajuan->id) }}" class="inline-flex items-center px-3 py-1.5 bg-primary-600 hover:bg-primary-700 text-white font-bold rounded-lg transition text-xs shadow-sm">
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="px-6 py-12 text-center">
                                        ...
                                        <h3 class="mt-2 text-sm font-semibold text-gray-900">Tidak ada pengajuan anggaran</h3>
                                        <p class="mt-1 text-xs text-gray-500 font-medium">Saat ini tidak ada berkas masuk dari Kepala Balai yang memerlukan verifikasi biaya.</p>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
